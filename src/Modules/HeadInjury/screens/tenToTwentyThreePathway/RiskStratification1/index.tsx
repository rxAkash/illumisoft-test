...
import {
   RISK_STRATIFICATION_ASSESSMENT_2_TEN,
...
const RiskStarticationTen = React.memo(({ navigation }: Props) => {
  const [ans, setAnswer] = useState<{ [key: string]: number }>({});

  const onNav = () => {
    navigation.navigate(RISK_STRATIFICATION_ASSESSMENT_2_TEN);
  };
  ...
  return (
    <HeadInjuryLayout
      title={headInjuryText.tenToTwentyThreePathway.RiskStartication.headerText}
      ...
      <>
        <View style={styles.titleContainer}>
          <Text style={styles.title}>
            {headInjuryText.tenToTwentyThreePathway.RiskStartication.title}
          </Text>
...
export default RiskStarticationTen;
