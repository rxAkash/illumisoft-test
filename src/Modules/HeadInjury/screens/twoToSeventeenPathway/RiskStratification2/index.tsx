...
import { PHYSICAL_EXAM, PHYSICAL_EXAM_TWO } from "Modules/HeadInjury/navigation/routes";
import { RISK_STRATIFICATION_ASSESSMENT } from "./conf";
interface Props {
  navigation: NavigationProp<any>;
}

const RiskStartication2Two = React.memo(({ navigation }: Props) => {
  const [ans, setAnswer] = useState<{ [key: string]: number }>({});

  const onNav = () => {
    navigation.navigate(PHYSICAL_EXAM_TWO);
  };
  useFocusEffect(
    React.useCallback(() => {
      setAnswer({});
  ...
  const renderBottomPanel = () => (
    <View style={styles.bottomContainer}>
      <ColoredNavButton
        title={headInjuryText.twoToSeventeenPathway.RiskStartication2.btnTitle}
  ...
  return (
    <HeadInjuryLayout
      title={headInjuryText.twoToSeventeenPathway.RiskStartication2.headerText}
...
export default RiskStartication2Two;
