...
import {
  RISK_STRATIFICATION_ASSESSMENT_2,
...
const RiskStartication = React.memo(({ navigation }: Props) => {
  const [ans, setAnswer] = useState<{ [key: string]: number }>({});

  const onNav = () => {
    navigation.navigate(RISK_STRATIFICATION_ASSESSMENT_2);
  };
  ...
  return (
    <HeadInjuryLayout
      title={headInjuryText.zeroToNinePathway.RiskStartication.headerText}
...
export default RiskStartication;
